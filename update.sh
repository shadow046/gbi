...
get_data(){
    ...
    then
        ...
        twodays="$(date +%m-%d-%Y --date='2 day ago')"
        if [ -f "$1.*" ]; then
            rm $1.*
        fi
        echo "Select Task.Id, Task.TaskNumber , Task.DateCreated, Task.TaskStatus, Form.Id as FormId, Task.CreatedBy From Task
        ...
        AND Format(Task.DateCreated, 'MM-dd-yyyy', 'en-US') <= '${twodays}';" > $1.sql
        if [ -f "$1.csv" ]; then
            rm $1.csv
            touch $1.csv
        else
        ...
        n=`cat $1.csv |wc -l`
        x=$((n-1))
        { echo "Taskid,TaskNumber,DateCreated,TaskStatus,FormId,CreatedBy"; sed "$x,\$d" $1.csv; } > $1new.csv
        # if [ -f "$1.sql" ]; then
        #     rm $1.sql
        #     touch $1.sql
        # else
        ...
        while IFS="," read -r TaskId TaskNumber DateCreated TaskStatus FormId CreatedBy
        do
            mysql -unuserv-demo -h 122.248.200.34 -D gbi -e "INSERT INTO Ticket 
                    (TaskId, TaskNumber, DateCreated, TaskStatus, CreatedBy)
                VALUES
                    ('$(echo ${TaskId})', '$(echo ${TaskNumber})', \"$(echo ${DateCreated%%.*})\", \"$(echo ${TaskStatus})\", \"$(echo ${CreatedBy})\")
