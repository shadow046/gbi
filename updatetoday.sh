...
get_data(){
    ...
    # DAYS="100"
        ...
        dt=$(date '+%d/%m/%Y %H:%M:%S');
        file=$(date '+%d-%m-%Y');
        echo "${1} Started at $dt" >> "$file update.txt"
        today="$(date '+%m-%d-%Y')"
        # today="$(date +%m-%d-%Y --date='2 day ago')"
        if [ -f "$1.*" ]; then
            rm $1.*
        fi
        echo "Select Task.Id, Task.TaskNumber , Task.DateCreated, Task.TaskStatus, FormField.FormId, FormField.value, Task.CreatedBy From Task
        ...
        And Formfield.Value != ''
        and Format(Task.DateCreated, 'MM-dd-yyyy', 'en-US') >= '${today}';" > $1.sql
        if [ -f "$1.csv" ]; then
            rm $1.csv
            touch $1.csv
        else
        ...
        do
            mysql -unuserv-demo -h 122.248.200.34 -D gbi -e "UPDATE Ticket 
                SET
                    TaskId = '$(echo ${TaskId})',
                    ...
                    FormId = \"$(echo ${FormId})\",
                    ${1:3} = \"$(echo ${Value})\",
                    CreatedBy = \"$(echo ${CreatedBy})\"
                WHERE TaskNumber = '$(echo ${TaskNumber})';"
        done < <(tail -n +2 $1new.csv)
        # sed -i 's/\x27Null\x27/NULL/gI' $1.sql
        # mysql -vv -unuserv-demo -pnuserv-demo -h 122.248.200.34 -D gbi < $1.sql
        rm $1*.*
        dt=$(date '+%d/%m/%Y %H:%M:%S');
        echo "${field} done at $dt" >> "$file update.txt"
}
getfields(){
    dt=$(date '+%d/%m/%Y %H:%M:%S');
    file=$(date '+%d-%m-%Y');
    echo "Backup Started at $dt" > "$file update.txt"
    ...
    dt=$(date '+%d/%m/%Y %H:%M:%S');
    echo "Script Stopped at $dt" >> "$file update.txt"
}
getfields